...
void RegisterHook(OUT Aurie::AurieStatus& status, IN const char* script_name, IN std::string_view hook_identifier, IN PVOID destination_function) {
...
*/
void RegisterHooks(OUT Aurie::AurieStatus& status, std::vector<std::tuple<const char*, std::string_view, PVOID>> hooks) {
